...
public class CentrifugeCategory extends BaseCategory<CentrifugeCategory.CentrifugeRecipeAdapter> {
    ...
    public CentrifugeCategory(IJeiHelpers helpers) {
        super(helpers.getGuiHelper(), RECIPE,
                ...
                helpers.getGuiHelper().createDrawableIngredient(VanillaTypes.ITEM_STACK, ModItems.CENTRIFUGE.get().getDefaultInstance()));
        this.helpers = helpers;
    }

    public static <T> List<CentrifugeRecipeAdapter> getRecipes(IPlatformFluidHelper<T> helpers, Collection<CentrifugeRecipe> recipes) {
        List<CentrifugeRecipeAdapter> newRecipes = new ArrayList<>();
        for (CentrifugeRecipe recipe : recipes) {
            newRecipes.add(new CentrifugeRecipeAdapter<>(helpers, recipe));
        }
        return newRecipes;
    }
    ...
    public @NotNull List<Component> getTooltipStrings(@NotNull CentrifugeRecipeAdapter recipe, @NotNull IRecipeSlotsView view, double mouseX, double mouseY) {
        for (int i = 0; i < 3; i++) {
            ...
            if (recipe.getRecipe().fluidOutputs().size() > i) {
                ...
                Optional<Pair<Double, Component>> output = recipe.getFluidWeight(this.helpers, view, id);
                if (output.isPresent()) {
                    double itemChance = recipe.getRecipe().fluidOutputs().get(i).chance();
                    List<Component> fluidTooltip = drawTooltip(output.get().second(), output.get().first(), itemChance, mouseX, mouseY, i, recipe.getRecipe().fluidOutputs().size(), 97, 106);
    ...
    protected static class CentrifugeRecipeAdapter<T> {

        private final CentrifugeRecipe recipe;

        private final HashMap<String, Object2DoubleMap<ItemStack>> itemWeights = new HashMap<>();
        private final HashMap<String, Object2DoubleMap<T>> fluidWeights = new HashMap<>();

        private final IPlatformFluidHelper<T> helper;

        public CentrifugeRecipeAdapter(IPlatformFluidHelper<T> helper, CentrifugeRecipe recipe) {
            this.recipe = recipe;
            this.helper = helper;
            ...
            for (int i = 0; i < fluidOutputList.size(); i++) {
                Object2DoubleMap<T> weights = new Object2DoubleOpenHashMap<>();
                fluidOutputList.get(i).pool().forEachWithSelf((pool, output) -> {
                    T fluid = helper.create(output.fluid().fluid(), output.fluid().amount(), output.fluid().tag());
        ...
        public Optional<Pair<Double, Component>> getFluidWeight(IJeiHelpers helpers, IRecipeSlotsView view, String id) {
            return view.findSlotByName(id)
                    .flatMap(slot -> slot.getDisplayedIngredient(this.helper.getFluidIngredientType()))
                    .map(fluid -> {
                        IIngredientHelper<T> helper = helpers.getIngredientManager().getIngredientHelper(fluid);
                        return Pair.of(this.getFluidWeight(id, fluid), Component.literal(helper.getDisplayName(fluid)));
                    });
        }

        public Double getFluidWeight(String slot, T displayedStack) {
            Map<T, Double> weightMap = fluidWeights.get(slot);
            if (weightMap == null) return null;
            for (Map.Entry<T, Double> entry : weightMap.entrySet()) {
                if (entry.getKey() == displayedStack) return entry.getValue();
            }
            return null;
        }
