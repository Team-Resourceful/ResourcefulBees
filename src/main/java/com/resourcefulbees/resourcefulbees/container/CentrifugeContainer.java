...
public class CentrifugeContainer extends Container {
    ...
    public CentrifugeContainer(int id, World world, BlockPos pos, PlayerInventory inv) {
        this(id, world, pos, inv, new IntArray(1));
    }

    public CentrifugeContainer(int id, World world, BlockPos pos, PlayerInventory inv, IntArray times) {
        ...
        if (centrifugeTileEntity != null) {
            this.addSlot(new SlotItemHandlerUnconditioned(centrifugeTileEntity.getItemStackHandler(), CentrifugeTileEntity.HONEYCOMB_SLOT, 30, 20) {
                public boolean isItemValid(ItemStack stack) {
                    return !stack.getItem().equals(Items.GLASS_BOTTLE);
                }
            });
            this.addSlot(new SlotItemHandlerUnconditioned(centrifugeTileEntity.getItemStackHandler(), CentrifugeTileEntity.BOTTLE_SLOT, 30, 38) {
                public boolean isItemValid(ItemStack stack) {
                    return stack.getItem().equals(Items.GLASS_BOTTLE);
                }
            });
            this.addSlot(new OutputSlot(centrifugeTileEntity.getItemStackHandler(), CentrifugeTileEntity.HONEY_BOTTLE, 80, 59));
            this.addSlot(new OutputSlot(centrifugeTileEntity.getItemStackHandler(), CentrifugeTileEntity.OUTPUT1, 129, 20));
            this.addSlot(new OutputSlot(centrifugeTileEntity.getItemStackHandler(), CentrifugeTileEntity.OUTPUT2, 129, 38));
        }
