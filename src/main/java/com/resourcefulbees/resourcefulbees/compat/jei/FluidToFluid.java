...
public class FluidToFluid implements IRecipeCategory<FluidToFluid.Recipe> {
                ...
                beeData.getMutationData().iBlockMutations.forEach((b, m) -> {
                    if (m.type == MutationTypes.FLUID_TO_FLUID) {
                        ...
                        if (input == null) {
                            ...
                            outputs.forEach(out -> {
                                double effectiveWeight = outputs.getAdjustedWeight(out.getValue().getWeight());
                                recipes.add(new Recipe(new FluidStack(input, 1000), new FluidStack(out.getKey(), 1000), beeData.getName(), m.type, effectiveWeight, out.getValue().getChance(), false));
                            });
                        }
                    }
                });
    ...
    public static class Recipe {
        ...
        public Recipe(FluidStack baseBlock, FluidStack mutationBlock, String beeType, MutationTypes type, double weight, double chance, boolean acceptsAny) {
        ...
        //TAGS!!!
        public Recipe(ITag<Fluid> baseBlock, FluidStack mutationBlock, String beeType, MutationTypes type, double weight, double chance, boolean acceptsAny) {
            this.weight = weight;
            this.chance = chance;
            this.fluidIn = null;
            this.fluidOut = mutationBlock;
