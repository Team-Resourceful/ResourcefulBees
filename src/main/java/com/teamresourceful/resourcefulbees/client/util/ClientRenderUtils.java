...
public final class ClientRenderUtils {
   ...
   /* public static void renderEntity(GuiGraphics graphics, Entity entity, float x, float y, float rotation, float renderScale) {
    ...
    public static void drawFluid(GuiGraphicsExtractor graphics, int height, int width, TextureAtlasSprite sprite, int color, int x, int y) {
        ...
        for (int i = 0; i < splits; i++) {
            ...
            graphics.blitSprite(RenderPipelines.GUI_TEXTURED, sprite, x, y + height - (splitHeight + (i * 16)), 0, splitHeight, width);
        }
