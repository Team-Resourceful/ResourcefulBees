...
public class UnvalidatedApiaryScreen extends AbstractContainerScreen<UnvalidatedApiaryContainer> {
    ...
    protected void init() {
        super.init();
        this.addWidget(new Button(getGuiLeft() + 116, getGuiTop() + 10, 50, 20, TranslationConstants.Apiary.VALIDATE_BUTTON, onPress -> this.validate()));
        BuildButton buildStructureButton = this.addWidget(new BuildButton(getGuiLeft() + 116, getGuiTop() + 35, 50, 20, TranslationConstants.Apiary.BUILD_BUTTON, onPress -> this.build()));
        if (!this.player.isCreative()) {
            buildStructureButton.active = false;
        }
        this.previewButton = this.addWidget(new PreviewButton(getGuiLeft() + 22, getGuiTop() + 25, 12, 12, 0, 24, 12, arrowButtonTexture, this.menu.getApiaryTileEntity().isPreviewed(), onPress -> {
            setPreviewToggle();
            previewSetToggle(this.previewButton.isTriggered());
        }));
        previewSetToggle(this.previewButton.isTriggered());
        this.upButton = this.addWidget(new ArrowButton(getGuiLeft() + 22, getGuiTop() + 12, ArrowButton.Direction.UP, onPress -> this.offsetPosition(Direction.UP)));
        this.downButton = this.addWidget(new ArrowButton(getGuiLeft() + 22, getGuiTop() + 38, ArrowButton.Direction.DOWN, onPress -> this.offsetPosition(Direction.DOWN)));
        this.leftButton = this.addWidget(new ArrowButton(getGuiLeft() + 9, getGuiTop() + 25, ArrowButton.Direction.LEFT, onPress -> this.offsetPosition(Direction.LEFT)));
        this.rightButton = this.addWidget(new ArrowButton(getGuiLeft() + 35, getGuiTop() + 25, ArrowButton.Direction.RIGHT, onPress -> this.offsetPosition(Direction.RIGHT)));
    }
    ...
    private void offsetPosition(Direction direction) {
        ...
        verticalOffset = Mth.clamp(verticalOffset, -1, 2);
        horizontalOffset = Mth.clamp(horizontalOffset, -2, 2);
    ...
    protected void renderBg(@NotNull PoseStack matrix, float partialTicks, int mouseX, int mouseY) {
        ...
        if (client != null) {
            RenderUtils.bindTexture(unvalidatedTexture);
    ...
    public class PreviewButton extends ImageButton {
        ...
        @Override
        public void renderToolTip(@NotNull PoseStack matrix, int mouseX, int mouseY) {
            TranslatableComponent s = isTriggered() ? TranslationConstants.Apiary.PREVIEW_DISABLED : TranslationConstants.Apiary.PREVIEW_ENABLED;
