...
public final class ModelHandler {
    ...
    public static void onAddAdditional(RegisterAdditionaModelsEvent event) {
        ...
        ModItems.HONEYCOMB_ITEMS.getEntries().forEach(comb -> registerGenericItem(event, comb, ResourcefulBees.id("honeycomb"), resourceManager));
        ModItems.HONEYCOMB_BLOCK_ITEMS.getEntries().forEach(combBlock -> registerGenericItem(event, combBlock, ResourcefulBees.id("honeycomb_block"), resourceManager));

        ModItems.HONEY_BLOCK_ITEMS.getEntries().forEach(honeyBlock -> registerGenericItem(event, honeyBlock, ResourcefulBees.id("honey_block"), resourceManager));
        ModItems.HONEY_BUCKET_ITEMS.getEntries().forEach(bucket -> registerGenericItem(event, bucket, ResourcefulBees.id("custom_honey_fluid_bucket"), resourceManager));
        ModItems.HONEY_BOTTLE_ITEMS.getEntries().forEach(bucket -> registerGenericItem(event, bucket, ResourcefulBees.id("honey_bottle"), resourceManager));

        ModItems.SPAWN_EGG_ITEMS.getEntries().forEach(egg -> registerGenericItem(event, egg,  ResourceLocation.withDefaultNamespace("template_spawn_egg"), resourceManager));

        ModBlocks.HONEYCOMB_BLOCKS.getEntries().forEach(combBlock -> registerGenericBlockState(event, combBlock, ResourcefulBees.id("honeycomb_block"), null, resourceManager));

        ModBlocks.HONEY_BLOCKS.getEntries().forEach(honeyBlock -> registerGenericBlockState(event, honeyBlock, ResourcefulBees.id("honey_block"), RenderType.translucent(), resourceManager));
    }
