...
public interface SyncableGUI extends MenuProvider {
    ...
    default void sendToPlayer(ServerPlayer player) {
        ...
        //todo NetworkHandler.CHANNEL.sendToPlayer(new SyncGuiPacket(this), player);
    }
    ...
    default void sendToPlayersTrackingChunk(){
        if (getLvl() == null || getLvl().isClientSide()) return;
        //todo NetworkHandler.CHANNEL.sendToAllLoaded(new SyncGuiPacket(this), getLvl(), getBlkPos());
    }
    ...
    default void sendToPlayersInRange(double range){
        if (getLvl() == null || getLvl().isClientSide()) return;
        //todo NetworkHandler.CHANNEL.sendToPlayersInRange(new SyncGuiPacket(this), getLvl(), getBlkPos(), range);
    }
    ...
    default void sendToListeningPlayers() {
        if (getLvl() == null || getLvl().isClientSide()) return;
        //todo NetworkHandler.CHANNEL.sendToPlayers(new SyncGuiPacket(this), getListeners());
    }
