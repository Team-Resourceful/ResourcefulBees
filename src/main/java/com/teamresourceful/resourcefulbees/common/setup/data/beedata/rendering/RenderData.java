...
public record RenderData(Set<BeeLayerData> layers, BeeColorData colorData, ResourceLocation model, BeeLayerTexture texture, ResourceLocation animations, float sizeModifier) implements BeeRenderData {
    ...
    private static final ResourceLocation BASE_MODEL = ResourcefulBees.id("geo/base.geo.json");
    private static final ResourceLocation BASE_ANIMATION = ResourcefulBees.id("animations/bee.animation.json");
    ...
    public static final BeeDataSerializer<BeeRenderData> SERIALIZER = BeeDataSerializer.of(ResourcefulBees.id("rendering"), 1, id -> CODEC, DEFAULT);
