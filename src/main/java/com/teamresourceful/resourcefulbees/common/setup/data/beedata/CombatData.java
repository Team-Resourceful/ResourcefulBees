...
public record CombatData(
    ...
    private static final Codec<BeeCombatData> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            Codec.unboundedMap(BuiltInRegistries.ATTRIBUTE.holderByNameCodec(), Codec.DOUBLE).optionalFieldOf("attributes", DEFAULT_ATTRIBUTES).forGetter(BeeCombatData::attributes)
    ).apply(instance, CombatData::new));
    public static final BeeDataSerializer<BeeCombatData> SERIALIZER = BeeDataSerializer.of(ResourcefulBees.id("combat"), 1, id -> CODEC, DEFAULT);
