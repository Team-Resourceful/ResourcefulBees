...
public record CombatData(
        ...
        Map<Attribute, Double> attributes //todo probably have to switch back to not using holder
    ...
    public static final BeeDataSerializer<BeeCombatData> SERIALIZER = BeeDataSerializer.of(ModIdentifier.of("combat"), 1, id -> CODEC, DEFAULT);

    @Override
    public AttributeSupplier.Builder buildAttributes(AttributeSupplier.Builder builder) {
        DEFAULT_ATTRIBUTES.forEach((attribute, aDouble) -> builder.add(Holder.direct(attribute), aDouble));
        attributes.forEach((attribute, aDouble) -> builder.add(Holder.direct(attribute), aDouble));
        return builder;
    }
