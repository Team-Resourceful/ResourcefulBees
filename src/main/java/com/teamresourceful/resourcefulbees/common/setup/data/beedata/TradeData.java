...
public record TradeData(
    ...
    public MerchantOffer getMerchantOffer(RandomSource random, ItemStack product, int flowerMin, int flowerMax) {
        ...
        if (!secondaryItem.isEmpty()) {
            secondCost = new ItemCost(
                secondaryItem.getItem().builtInRegistryHolder(),
                secondaryItemCost().sample(random),
                DataComponentPredicate.allOf(secondaryItem.getComponents())
            );
        }

        return new MerchantOffer(
            new ItemCost(ModItems.GOLD_FLOWER_ITEM.get(), random.nextIntBetweenInclusive(flowerMin, flowerMax)),
            Optional.ofNullable(secondCost),
            product,
    ...
    public static final BeeDataSerializer<BeekeeperTradeData> SERIALIZER = BeeDataSerializer.of(ResourcefulBees.id("trade"), 1, id -> CODEC, DEFAULT);
